...
Class Helmholtz uses cant::sample_m for float or double. Depending on the
context where the class is used, you may need to define cant::sample_m. If used
with PD or MaxMsp, it is already defined.
...
public:
  /** -- methods  -- **/
  CANT_EXPLICIT Helmholtz(size_u frameSize, type_i overlap = c_defaultOverlap,
...
private:
  ...
  // functions
  CANT_NODISCARD static sample_f interpolate3max(const sample_f *buf,
                                                 size_u peakIndex);
  CANT_NODISCARD static sample_f interpolate3phase(const sample_f *buf,
                                                   size_u peakIndex);
  ...
  // constants
  static CANT_CONSTEXPR type_i c_defaultFrameSize =
      1024; // default analysis framesize
  ...
  static CANT_CONSTEXPR type_d c_defaultSeekLengthRatio =
      0.85; // seek-length as ratio of framesize
};
