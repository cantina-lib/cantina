...
Helmholtz::Helmholtz(size_u frameSize, type_i overlap, type_d bias)
    : m_fftPerformer(2 * frameSize), m_timeIndex(0), m_periodIndex(0),
      m_minRMS(c_defaultMinRMS), m_inputBuffer(), m_inputBuffer2(),
      m_processBuffer(), m_periodLength(0.), m_fidelity(0.) {
  ...
  m_processBuffer =
      std::vector<sample_f>(2 * static_cast<size_u>(m_frameSize), 0.);
}
...
void Helmholtz::inSamples(const sample_f *in, size_u blockSize) {
...
void Helmholtz::inOutSamples(const sample_f *in, sample_f *out,
                             size_u blockSize) {
  int mask = static_cast<int>(m_frameSize) - 1;
  int size = static_cast<int>(blockSize);
  int outIndex = 0;
...
void Helmholtz::setFrameSize(size_u frameSize) {
  if (!((frameSize == 128) || (frameSize == 256) || (frameSize == 512) ||
        (frameSize == 1024) || (frameSize == 2048))) {
...
CANT_NODISCARD
size_u Helmholtz::getFrameSize() const { return m_frameSize; }
...
void Helmholtz::setBias(type_d bias) {
  if (bias > 1.)
    bias = 1.;
  if (bias < 0.)
    bias = 0.;
...
void Helmholtz::setMinRMS(type_d rms) {
  if (rms > 1.)
    rms = 1.;
  if (rms < 0.)
    rms = 0.;
...
CANT_NODISCARD
type_d Helmholtz::getPeriod() const {
  return static_cast<sample_f>(m_periodLength);
}

CANT_NODISCARD
type_d Helmholtz::getFidelity() const {
...
void Helmholtz::analyzeFrame() {
  ...
  for (n = 0; n < m_frameSize; n++) {
    m_processBuffer.at(n) =
        m_inputBuffer.at(static_cast<size_u>(tindex++ & mask)) * norm;
  }

  // copy for normalization function
  for (n = 0; n < m_frameSize; n++) {
    m_inputBuffer2.at(n) =
        m_inputBuffer.at(static_cast<size_u>(tindex++ & mask));
  }
  // zeropadding
  std::fill(m_processBuffer.begin() + static_cast<long int>(m_frameSize),
            m_processBuffer.end(), static_cast<sample_f>(0.));
...
void Helmholtz::autoCorrelation() {
  ...
  for (n = 1; n < m_frameSize; n++) {
    ...
    m_processBuffer[n] =
        m_processBuffer.at(n) * m_processBuffer.at(n) +
...
void Helmholtz::normalize() {
  ...
  type_d normIntegral;
  {
    ...
    const auto rms = static_cast<sample_f>(
  ...
  std::fill(m_processBuffer.begin() + static_cast<unsigned int>(seek),
...
void Helmholtz::pickPeak() {
  const auto seek = static_cast<size_u>(std::floor(
      static_cast<sample_f>(m_frameSize) * c_defaultSeekLengthRatio));
  const auto bias = static_cast<sample_f>(
      m_biasFactor /
      static_cast<sample_f>(m_frameSize)); // user-controlled bias

  // skip main lobe
  auto n = static_cast<size_u>(std::round(
      std::distance(m_processBuffer.begin(),
                    std::find_if(m_processBuffer.begin(), m_processBuffer.end(),
                                 [](const auto &e) { return e < 0.; }))));
  // find interpolated / biased maximum in specially normalized autocorrelation
  // function interpolation finds the 'real maximum' biasing favours the first
  // candidate
  sample_f maxValue = 0.;
  sample_f realPeak;
  size_u peakIndex = 0;
  for (; n < seek - 1; n++) {
    if ((m_processBuffer.at(n) > m_processBuffer.at(n - 1)) &&
        (m_processBuffer.at(n) > m_processBuffer.at(n + 1))) {
      /* local peak */
      realPeak = interpolate3max(m_processBuffer.data(), n);

      if ((realPeak * (1. - static_cast<sample_f>(static_cast<type_d>(n) *
                                                  bias))) > maxValue) {
        maxValue = realPeak;
        peakIndex = n;
      }
...
void Helmholtz::periodAndFidelity() {
  if (m_periodIndex) {
    m_periodLength = static_cast<sample_f>(m_periodIndex) +
                     interpolate3phase(m_processBuffer.data(), m_periodIndex);
...
CANT_NODISCARD
sample_f Helmholtz::interpolate3max(const sample_f *buf, size_u peakIndex) {
  sample_f realpeak;
  ...
  realpeak = static_cast<sample_f>(b + 0.5 * (0.5 * ((c - a) * (c - a))) /
                                           (2 * b - a - c));
  return realpeak;
}

CANT_NODISCARD
sample_f Helmholtz::interpolate3phase(const sample_f *buf, size_u peakIndex) {
  sample_f fraction;
