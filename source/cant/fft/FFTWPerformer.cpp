...
FFTWPerformer::FFTWPerformer(const size_u transformSize)
...
fftwf_plan FFTWPerformer::computeRealPlan(FFTW3FBlock &inOutBuffer,
                                          fftw_r2r_kind kind) {
  return fftwf_plan_r2r_1d(static_cast<int>(inOutBuffer.size()),
                           inOutBuffer.data(), inOutBuffer.data(), kind,
                           FFTW_PATIENT);
}
...
void FFTWPerformer::performReal(Stream<sample_f> &inout,
                                const fftwf_plan &realPlan) {
  /*
...
void FFTWPerformer::performRealForward(Stream<sample_f> &inout) {
...
void FFTWPerformer::performRealInverse(Stream<sample_f> &inout) {
  /*
  for (size_u i = getTransformSize() / 2; i < getTransformSize(); ++i)
  {
      inout.at(i) = - inout.at(i);
