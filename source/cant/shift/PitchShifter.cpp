...
CANTINA_CANT_NAMESPACE_BEGIN
    ...
    bool
    PitchShifter::
    shouldTrimBuffers(const pan::MidiNoteOutput &note, time_d maxLatency) const
    {
    ...
    size_u
    PitchShifter::
    timeToNumberSamples(time_d t) const
    {
        return
            static_cast<size_u>(
                    std::round(
                    (t / static_cast<time_d>(1000))
                    * static_cast<time_d>(getSampleRate())
                    )
            );
    }
