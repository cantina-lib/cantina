...
SoundTouchShifter::SoundTouchShifter(const size_u numberVoices,
                                     const type_i sampleRate,
                                     const type_i sequence)
    : m_sampleRate(sampleRate), m_touches(numberVoices) {
...
void SoundTouchShifter::clearBuffers(const size_u voice) {
  m_touches.at(voice).clear();
}

void SoundTouchShifter::trimBuffers(const size_u voice,
                                    const size_u numberSamples) {
  m_touches.at(voice).adjustAmountOfSamples(static_cast<uint>(numberSamples));
}

size_u SoundTouchShifter::getNumberSamplesAvailable(const size_u voice) const {
...
void SoundTouchShifter::update(size_u voice, const sample_f *input,
...
size_u SoundTouchShifter::getNumberVoices() const { return m_touches.size(); }
